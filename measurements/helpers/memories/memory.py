...
# consider create two memory classes, one for spokes, another for the hub
class Memory:
    def __init__(self, name):
        db_url = Configs.db_url
        llm=ChatOpenAI(model='gpt-4', temperature=0.0, model_kwargs={"seed": 0}) #OpenAI

        # Set up a database for storing memory content
        ...
        # Set up full conversation memory, stored in the database
        ...
        # Set up summarized memory for prompt use
        ...
        # Set up entity memory, stored in the database
    ...
    def get_cov_memory(self):
        for item in self.memory.memories:
            if isinstance(item, ConversationBufferMemory):
                return item
    ...
    # Each type of memory has it own save_context method with passing inputs and outputs dictionaries
    # For example:
    def record_history(self, inputs, outputs):
        inputs_dict = {"input": inputs}  
        outputs_dict = {"output": outputs} #{"text": outputs} #
        conv_memory = self.get_cov_memory()
        conv_memory.save_context(inputs_dict, outputs_dict)
